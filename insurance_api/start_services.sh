...
if check_port 8000; then
    ...
    echo -e "${YELLOW}[*] 启动 vLLM 服务 (端口 8000，GPU 1)...${NC}"
    CUDA_VISIBLE_DEVICES=1 conda run -n baichuan-chat python -m vllm.entrypoints.openai.api_server \
...
if check_port 8001; then
    ...
    python -m uvicorn main_api:app --host 0.0.0.0 --port 8001 >backend_run.log 2>&1 &
...
if check_port 8095; then
    ...
    BACKEND_BASE="http://127.0.0.1:8001" python ui_proxy_server.py >frontend_run.log 2>&1 &
