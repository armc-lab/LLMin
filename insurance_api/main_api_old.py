...
async def handle_chat_completion(request: ChatRequest):
    ...
    for i in ordered:
        if i < 0 or i >= len(all_chunks) or i in used:
            continue
        # 对于规则优先的块，没有相似度；Chroma 检索块从 sim_map 读取相似度
        if i in prio:
            seg = f"[规则优先]\n{all_chunks[i]}"
        else:
            # 从 sim_map 获取相似度
            s = sim_map.get(i, 0.40)  # 如果 i 不在 sim_map 中，默认给 0.40（边界）
            if s < SIM_THRESHOLD:
                continue
            seg = f"[相似度:{s:.2f}]\n{all_chunks[i]}"
