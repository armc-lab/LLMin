  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .chat-input-bar:focus-within {
      border-color: #b7c5a8;
      box-shadow: 0 6px 14px rgba(76, 116, 44, 0.12);
    }
    ...
    .backend-dot.ok { background: #22c55e; }
    .backend-dot.fail { background: #ef4444; }
...
<body>
  ...
  <script>
    ...
    async function checkBackendHealth() {
      ...
      const txt = $("backendBarText");
      if (dot) dot.className = "backend-dot";
      if (txt) txt.textContent = "正在检查后端连接...";
      try {
        const resp = await fetch("/api/backend-health", { cache: "no-store" });
        const data = await resp.json().catch(() => ({}));
        if (resp.ok && data && data.ok) {
          if (dot) dot.className = "backend-dot ok";
          if (txt) txt.textContent = "后端已连接，可正常上传与问答";
          return true;
        }
        if (dot) dot.className = "backend-dot fail";
        if (txt) txt.textContent = "后端未就绪，请先运行 run_all_ui_opt.sh";
        return false;
      } catch (_e) {
        if (dot) dot.className = "backend-dot fail";
        if (txt) txt.textContent = "后端连接失败，请检查后端进程";
        return false;
      }
    }
