...
VLLM_MODEL_PATH = os.getenv("VLLM_MODEL_PATH", "/data2/wangliangmin/snap/Baichuan")
...
EMBEDDING_MODEL_DIR = os.getenv(
    "EMBEDDING_MODEL_DIR",
    "/data2/wangliangmin/sherry/llm_safty_end2end/src/model/text2vec-base-chinese",
)
...
print(f"Chroma 客户端已初始化，数据路径: {CHROMA_DB_PATH}")
...
async def call_vllm(prompt: str, temperature: float = 0.0) -> str:
    ...
    try:
        ...
        # 带上响应体片段，便于快速定位 vLLM 的请求参数问题。
        err_body = (e.response.text or "")[:300]
        raise HTTPException(status_code=502, detail=f"vLLM 返回异常状态: {e.response.status_code}, body={err_body}")
...
def split_by_length(text: str, max_len: int = 800) -> List[str]:
    ...
    for para in text.splitlines():
        ...
        if cur_len + len(para) <= max_len:
            buf.append(para)
        else:
            if buf:
                out.append("\n".join(buf))
                buf = []
            # 超长段落按 max_len 切分，避免出现空分块并降低检索噪声。
            if len(para) > max_len:
                for i in range(0, len(para), max_len):
                    piece = para[i:i + max_len].strip()
                    if piece:
                        out.append(piece)
            else:
                buf = [para]
...
def extract_query_terms(question: str, query_hint: str = "") -> List[str]:
    """抽取用于词法召回的关键词，兼顾数字条款与中文术语。"""
    merged = f"{question} {query_hint}".strip()
    if not merged:
        return []

    # 条款号、章节号、以及 2~8 长度中文词片段。
    num_terms = re.findall(r"\d+(?:\.\d+)+|第\d+章|\d+日", merged)
    zh_terms = re.findall(r"[\u4e00-\u9fff]{2,8}", merged)

    stop_words = {
        "如果", "是否", "可以", "需要", "根据", "合同里", "合同中", "请问", "一下", "这个", "那个", "什么", "如何", "怎么", "我们",
    }
    terms: List[str] = []
    for t in num_terms + zh_terms:
        s = t.strip()
...
def lexical_overlap_score(chunk: str, query_terms: List[str]) -> float:
    """基于关键词命中计算词法得分。"""
    ...
    for t in query_terms:
        ...
        if cnt <= 0:
            continue
        # 标题/前部命中更有价值。
        bonus = 1.5 if t in head else 1.0
        # 数字条款号权重更高。
        num_w = 1.8 if re.search(r"\d", t) else 1.0
        score += min(cnt, 3) * bonus * num_w
    return score


def build_citation_snippet(chunk: str, query_terms: List[str], max_len: int = 300) -> str:
    """围绕命中的关键词截取引用片段，提升引用关键词可见性。"""
    s = chunk.replace("\n", " ").strip()
    if not s:
        return ""
